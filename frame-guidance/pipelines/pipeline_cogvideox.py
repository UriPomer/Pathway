...
from diffusers.utils import is_torch_xla_available, logging, replace_example_docstring, export_to_gif
...
class CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def _validate_guidance_args(self, guidance_step, guidance_lr, num_inference_steps, loss_fn):
        VALID_LOSS_FNS = {"frame", "style", "scribble", "gray", "depth", "loop"}
    ...
    def _prepare_loss_context(self, loss_fn, video, additional_inputs, device, dtype,
        ...
        el        if loss_fn in ("scribble", "gray"):
            ...
            ctx["dummy_video"] = [
                Image.new("RGB", (width, height), color="white") for _ in range(num_frames)
            ]
    ...
    def _predict_noise(
        ...
        image_rotary_emb, attention_kwargs, *, enable_grad=False,
    ):
    ...
    # ------------------------------------------------------------------ #
    #  Helper: compute loop loss (first vs last frame consistency)
    # ------------------------------------------------------------------ #
    def _compute_loop_loss(self, pred_original_sample, fixed_frames):
        """Decode first & last frames and return MSE between them."""
        predicted_images = []
        for fixed_frame in fixed_frames:
            if fixed_frame == 0:
                latent_pair = pred_original_sample[:, 0:1]
                decoded = self.decode_latents(latent_pair)
                predicted_images.append(decoded[:, :, 0:1])
            else:
                ...
                predicted_images.append(decoded[:, :, relative_idx : relative_idx + 1])

        loss = F.mse_loss(predicted_images[0].detach(), predicted_images[1])
        del predicted_images, decoded
    ...
    def __call__(
        ...
        if loss_fn == "loop":
            fixed_frames = loss_ctx["fixed_frames"]
            video = loss_ctx["dummy_video"]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                for rep in range(n_repeats + 1):
                    ...
                    if need_grad:
                        if loss_fn == "loop":
                            total_loss = self._compute_loop_loss(pred_original_sample, fixed_frames)
                        else:
                            fixed_frames_ = fixed_frames if fixed_frames is not None \
                                else random.sample(range(5, 49), 4)
                            total_loss = 0.0
                        ...
                        in_travel = (i + shorten_steps) >= travel_time[0] and \
                                    (i + shorten_steps) <= travel_time[1]
