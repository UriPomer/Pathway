...
"""
Patched WanImageToVideoPipeline — adds ``condition`` to callback_on_step_end
so that Temporal Latent Dropout (TLD) can be applied via callback without
rewriting the denoising loop.

Changes vs upstream diffusers (0.33.x):
  1. _callback_tensor_inputs += "condition"
  2. callback return-path pops "condition" back into the local variable
  3. Relative imports replaced with absolute imports from `diffusers`
"""
...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    # PATCH: added "condition" so TLD callback can modify it
    _callback_tensor_inputs = ["latents", "condition", "prompt_embeds", "negative_prompt_embeds"]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if callback_on_step_end is not None:
                    ...
                    # PATCH: allow callback to modify condition (for TLD)
                    condition = callback_outputs.pop("condition", condition)
